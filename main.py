...
def process_video(video_url):
    ...
    # Calcular dimensions parelles per a 1080p (Evita l'error de Broken Pipe)
    target_h = 1080
    w, h = clip.size
    target_w = int(w * (target_h / h))
    if target_w % 2 != 0: target_w -= 1

    # Redimensionar segons versió de MoviePy (v1 o v2)
    if hasattr(clip, "resized"):
        clip = clip.resized(new_size=(target_w, target_h))
    else:
        clip = clip.resize(new_size=(target_w, target_h))

    # Escriptura amb còdecs de màxima compatibilitat mòbil
...
def main():
    ...
    video_enviat = False
    # Intentem amb 2 comptes diferents si el primer no té res nou
    for _ in range(2):
        ...
        try:
            ...
            # Filtrem vídeos nous i triem el més viral (més likes)
